...
- `facts/` を追加・変更したら `make sync-generated` を実行し、更新された `src/` と `generated/` を同じコミットに含める
- CI の `Ensure generated files are committed` は `src/` と `generated/` の tracked / untracked 両方を確認する
...
`facts/` を触ったコミットでは、少なくとも `make sync-generated` を実行してから commit する。テンプレート変更だけなら `cargo run -p factctl -- build-pages` か `make build-pages` でよい。
